...
# ------------------------------------------------------------------
# STATUS / PORTAS (CORRIGIDO)
# Antes usava .inbounds[0].port, que é sempre a porta 2000 (API
# interna do Xray), não a porta real de tráfego dos clientes — por
# isso a porta exibida nunca batia com a porta configurada de fato.
# Agora ignora o inbound "dokodemo-door" (api) e mostra TODAS as
# portas de tráfego real, cruzando com o que está de fato em LISTEN.
# ------------------------------------------------------------------
draw_status() {
...
# ------------------------------------------------------------------
# CRIAR USUÁRIO XRAY — agora em sincronia total com adduser.sh e Atlas
# Antes esta função só injetava no config.json do Xray e não gravava
# em usuarios.db nem avisava o Atlas. Resultado: o usuário aparecia
# funcionando na hora, mas sumia da lista principal do painel (opção
# 04), não tinha data de validade controlada pelo limiter/expirador,
# e não existia no painel Atlas (DragonCore) — três painéis, três
# realidades diferentes para o mesmo cliente.
# ------------------------------------------------------------------
add_user_xray() {
    clear
    draw_status
    ...
    # ---- Atlas API (pra existir também no painel DragonCore) ----
    if command -v atlas_criar_user &>/dev/null; then
        echo -ne "${C}[ATLAS] Sincronizando com o painel... ${NC}"
        atlas_resp=$(atlas_criar_user "$nick" "$pass" "$dias" "$limite" "" 2>&1)
    ...
    echo -e "${G}✅ USUÁRIO XRAY CRIADO E SINCRONIZADO!${NC}"
    echo -e "${W}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
...
# Na tela inicial (menu.sh, opção 04) o UUID aparece cortado
...
uninstall_xray() {
    ...
    echo -e "${R}Os usuários continuam cadastrados em $USERDB e no Atlas,${NC}"
...
while true; do
    clear
    echo -e "${C}  🛰️  NETSIMON 4.0 - XRAY MANAGER${NC}"
    draw_status
